    ...
    <div class="p-1.5 min-w-full inline-block align-middle">
        ...
        <div class="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-b border-gray-200 dark:border-neutral-700">
            ...
            <div class="inline-flex gap-x-2">
                ...
                <a class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-transparent bg-red-600 text-white hover:bg-red-700 focus:outline-hidden focus:bg-red-700 disabled:opacity-50 disabled:pointer-events-none" href="{{ route('grades.create') }}">
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-neutral-700" data-searchable="true">
            ...
            <tr class="bg-white hover:bg-gray-50 dark:bg-neutral-900 dark:hover:bg-neutral-800">
                ...
                <button type="button" class="block text-start" aria-haspopup="dialog" aria-expanded="false" aria-controls="hs-grade-modal-{{ $grade->id }}" data-hs-overlay="#hs-grade-modal-{{ $grade->id }}">
                    ...
                    <div class="flex items-center gap-x-3">
                        ...
                        <span class="text-sm font-medium text-red-800 dark:text-red-400">
                            {{ substr($grade->student->name ?? 'N/A', 0, 1) }}
                        </span>
                        </div>
                        <div class="grow">
                        <span class="block text-sm font-semibold text-gray-800 dark:text-neutral-200">{{ $grade->student->name ?? 'N/A' }}</span>
                        <span class="block text-xs text-gray-600 dark:text-neutral-400">{{ $grade->student->nim ?? 'N/A' }}</span>
                        </div>
                    </div>
                    </span>
                </button>
                ...
                <button type="button" class="block text-start" aria-haspopup="dialog" aria-expanded="false" aria-controls="hs-grade-modal-{{ $grade->id }}" data-hs-overlay="#hs-grade-modal-{{ $grade->id }}">
                    <span class="block px-6 py-2">
                    <span class="text-sm text-gray-600 dark:text-neutral-400">
                        {{ $grade->placement->company->name ?? 'N/A' }}
                    </span>
                    </span>
                </button>
                </td>
                ...
                <div class="px-6 py-1.5 flex justify-end gap-x-2">
                    ...
                    <a href="{{ route('grades.edit', $grade->id) }}" class="py-1 px-2 inline-flex justify-center items-center gap-2 rounded-lg border border-gray-200 font-medium bg-white text-gray-700 shadow-2xs align-middle hover:bg-gray-50 focus:outline-hidden focus:ring-2 focus:ring-offset-2 focus:ring-offset-white focus:ring-red-600 transition-all text-sm dark:bg-neutral-900 dark:hover:bg-neutral-800 dark:border-neutral-700 dark:text-neutral-400 dark:hover:text-white dark:focus:ring-offset-gray-800">
                    <svg class="shrink-0 size-4" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
            ...
            <!-- Grade Modal -->
            <div id="hs-grade-modal-{{ $grade->id }}" class="hs-overlay hidden size-full fixed top-0 start-0 z-80 overflow-x-hidden overflow-y-auto pointer-events-none" role="dialog" tabindex="-1" aria-labelledby="hs-grade-modal-label-{{ $grade->id }}">
                ...
                <div class="relative flex flex-col bg-white shadow-lg rounded-xl pointer-events-auto dark:bg-neutral-800">
                    <div class="relative overflow-hidden min-h-32 bg-gradient-to-r from-red-600 to-purple-700 text-center rounded-t-xl">
                    <!-- Close Button -->
                    <div class="absolute top-2 end-2">
                        <button type="button" class="flex justify-center items-center size-7 text-sm font-semibold rounded-full border border-transparent text-white/70 hover:bg-white/10 focus:outline-hidden focus:bg-white/10 disabled:opacity-50 disabled:pointer-events-none" aria-label="Close" data-hs-overlay="#hs-grade-modal-{{ $grade->id }}">
                    ...
                    <!-- Icon -->
                    <span class="mx-auto flex justify-center items-center size-15.5 rounded-full border-4 border-white bg-gradient-to-r from-red-600 to-purple-700 text-white shadow-2xs dark:border-neutral-800">
                    ...
                    <div class="text-center">
                        <h3 id="hs-grade-modal-label-{{ $grade->id }}" class="text-lg font-semibold text-gray-800 dark:text-neutral-200">
                        Grade Details
                        </h3>
                        <p class="text-sm text-gray-500 dark:text-neutral-500">
                        Score: {{ $grade->score }} | Grade: {{ $gradeLetter }}
                        </p>
                    </div>
                    ...
                    <div class="mt-5 sm:mt-10 grid grid-cols-2 sm:grid-cols-3 gap-5">
                        ...
                        <span class="block text-sm font-medium text-gray-800 dark:text-neutral-200">{{ $grade->student->name ?? 'N/A' }}</span>
                        <span class="block text-xs text-gray-600 dark:text-neutral-400">{{ $grade->student->nim ?? 'N/A' }}</span>
                        </div>

                        <div>
                        <span class="block text-xs uppercase text-gray-500 dark:text-neutral-500">Teacher:</span>
                        <span class="block text-sm font-medium text-gray-800 dark:text-neutral-200">{{ $grade->teacher->name ?? 'N/A' }}</span>
                        </div>
                        ...
                        <div>
                        <span class="block text-xs uppercase text-gray-500 dark:text-neutral-500">Grade:</span>
                        ...
                        <span class="block text-sm font-medium text-gray-800 dark:text-neutral-200">{{ $grade->placement->company->name ?? 'N/A' }}</span>
                        </div>
                        ...
                        <span class="block text-sm font-medium text-gray-800 dark:text-neutral-200">
                            @if($grade->score >= 70)
                            <span class="py-1 px-2 inline-flex items-center gap-x-1 text-xs font-medium bg-green-100 text-green-800 rounded-full">Passed</span>
                            @else
                            <span class="py-1 px-2 inline-flex items-center gap-x-1 text-xs font-medium bg-red-100 text-red-800 rounded-full">Failed</span>
                            @endif
                        </span>
                        </div>
                    </div>
                    ...
                    <div class="mt-5 sm:mt-10">
                        <h4 class="text-xs font-semibold uppercase text-gray-800 dark:text-neutral-200">Feedback</h4>
                    ...
                    <div class="mt-5 sm:mt-10">
                        <h4 class="text-xs font-semibold uppercase text-gray-800 dark:text-neutral-200">Performance</h4>
                        <div class="mt-3 w-full bg-gray-200 rounded-full h-2.5 dark:bg-neutral-700">
                        <div class="h-2.5 rounded-full 
                            @if($grade->score >= 85) bg-green-600
                            @elseif($grade->score >= 70) bg-red-600
                            @elseif($grade->score >= 60) bg-yellow-600
                            @else bg-red-600
                            @endif" 
                            style="width: {{ $grade->score }}%">
                        </div>
                        </div>
                        <div class="mt-2 flex justify-between text-xs text-gray-600 dark:text-neutral-400">
                        <span>0</span>
                        <span>50</span>
                        <span>100</span>
                        </div>
                    ...
                    <div class="mt-5 flex justify-end gap-x-2">
                        <a class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 shadow-2xs hover:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none focus:outline-hidden focus:bg-gray-50 dark:bg-transparent dark:border-neutral-700 dark:text-neutral-300 dark:hover:bg-neutral-800 dark:focus:bg-neutral-800" href="{{ route('grades.edit', $grade->id) }}">
                        <svg class="shrink-0 size-4" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
                            <path d="M12.146.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1 0 .708l-10 10a.5.5 0 0 1-.168.11l-5 2a.5.5 0 0 1-.65-.65l2-5a.5.5 0 0 1 .11-.168l10-10zM11.207 2.5 13.5 4.793 14.793 3.5 12.5 1.207 11.207 2.5zm1.586 3L10.5 3.207 4 9.707V10h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.293l6.5-6.5zm-9.761 5.175-.106.106-1.528 3.821 3.821-1.528.106-.106A.5.5 0 0 1 5 12.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.468-.325z"/>
                        </svg>
                        Edit Grade
                        </a>
                        <a class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-transparent bg-red-600 text-white hover:bg-red-700 focus:outline-hidden focus:bg-red-700 disabled:opacity-50 disabled:pointer-events-none" href="{{ route('grades.show', $grade->id) }}">
                        <svg class="shrink-0 size-4" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
                            <path d="M1.92.506a.5.5 0 0 1 .434.14L3 1.293l.646-.647a.5.5 0 0 1 .708 0L5 1.293l.646-.647a.5.5 0 0 1 .708 0L7 1.293l.646-.647a.5.5 0 0 1 .708 0L9 1.293l.646-.647a.5.5 0 0 1 .708 0l.646.647.646-.647a.5.5 0 0 1 .708 0l.646.647.646-.647a.5.5 0 0 1 .801.13l.5 1A.5.5 0 0 1 15 2v12a.5.5 0 0 1-.053.224l-.5 1a.5.5 0 0 1-.8.13L13 14.707l-.646.647a.5.5 0 0 1-.708 0L11 14.707l-.646.647a.5.5 0 0 1-.708 0L9 14.707l-.646.647a.5.5 0 0 1-.708 0L7 14.707l-.646.647a.5.5 0 0 1-.708 0L5 14.707l-.646.647a.5.5 0 0 1-.708 0L3 14.707l-.646.647a.5.5 0 0 1-.801-.13l-.5-1A.5.5 0 0 1 1 14V2a.5.5 0 0 1 .053-.224l.5-1a.5.5 0 0 1 .367-.27zm.217 1.338L2 2.118v11.764l.137.274.51-.51a.5.5 0 0 1 .707 0l.646.647.646-.646a.5.5 0 0 1 .708 0l.646.646.646-.646a.5.5 0 0 1 .708 0l.646.646.646-.646a.5.5 0 0 1 .708 0l.646.646.646-.646a.5.5 0 0 1 .708 0l.646.646.646-.646a.5.5 0 0 1 .708 0l.509.509.137-.274V2.118l-.137-.274-.51.51a.5.5 0 0 1-.707 0L12 1.707l-.646.647a.5.5 0 0 1-.708 0L10 1.707l-.646.647a.5.5 0 0 1-.708 0L8 1.707l-.646.647a.5.5 0 0 1-.708 0L6 1.707l-.646.647a.5.5 0 0 1-.708 0L4 1.707l-.646.647a.5.5 0 0 1-.708 0l-.509-.51z"/>
                            <path d="M3 4.5a.5.5 0 0 1 .5-.5h6a.5.5 0 1 1 0 1h-6a.5.5 0 0 1-.5-.5zm0 2a.5.5 0 0 1 .5-.5h6a.5.5 0 1 1 0 1h-6a.5.5 0 0 1-.5-.5zm0 2a.5.5 0 0 1 .5-.5h6a.5.5 0 1 1 0 1h-6a.5.5 0 0 1-.5-.5zm0 2a.5.5 0 0 1 .5-.5h6a.5.5 0 0 1 0 1h-6a.5.5 0 0 1-.5-.5zm8-6a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 0 1h-1a.5.5 0 0 1-.5-.5zm0 2a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 0 1h-1a.5.5 0 0 1-.5-.5zm0 2a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 0 1h-1a.5.5 0 0 1-.5-.5zm0 2a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 0 1h-1a.5.5 0 0 1-.5-.5z"/>
                        </svg>
        ...
        <div class="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-t border-gray-200 dark:border-neutral-700">
            ...
            <div class="inline-flex gap-x-2">
                ...
                @else
                <a href="{{ $grades->previousPageUrl() }}" class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 shadow-2xs hover:bg-gray-50 focus:outline-hidden focus:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-800 dark:border-neutral-700 dark:text-white dark:hover:bg-neutral-700 dark:focus:bg-neutral-700">
                <svg class="size-3" width="16" height="16" viewBox="0 0 16 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M10.506 1.64001L4.85953 7.28646C4.66427 7.48172 4.66427 7.79831 4.85953 7.99357L10.506 13.64" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
                </svg>
                Prev
                ...
                <a href="{{ $grades->nextPageUrl() }}" class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 shadow-2xs hover:bg-gray-50 focus:outline-hidden focus:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-800 dark:border-neutral-700 dark:text-white dark:hover:bg-neutral-700 dark:focus:bg-neutral-700">
                Next
        ...
        <div class="max-w-sm w-full min-h-100 flex flex-col justify-center mx-auto px-6 py-4">
            ...
            <a href="{{ route('grades.create') }}" class="py-2 px-3 inline-flex justify-center items-center gap-x-2 text-sm font-medium rounded-lg border border-transparent bg-red-600 text-white hover:bg-red-700 focus:outline-hidden focus:bg-red-700 disabled:opacity-50 disabled:pointer-events-none">
                <svg class="shrink-0 size-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14"/><path d="M12 5v14"/></svg>
                Add new grade
            </a>
            <button type="button" class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 shadow-2xs hover:bg-gray-50 focus:outline-hidden focus:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-800 dark:border-neutral-700 dark:text-white dark:hover:bg-neutral-700 dark:focus:bg-neutral-700">
